...
```bash
python3 /root/.openclaw/skills/minimax-image-t2i/scripts/minimax-image.py "用户想要的图片描述" -n 数量 -a 宽高比 -o 输出文件.png
```
...
5. Send the generated image back to user via 飞书 with detailed message:
   ...
   ```bash
   openclaw message send --channel feishu --message "🎨 图片生成完成！\n\n📝 原始提示词：{原始提示词}\n\n✨ 扩写后提示词：\n{完整的扩写后英文提示词}\n\n---\nGenerated by MiniMax AI" --media ~/minimax-images/输出文件.png
   ```
...
All images are saved to `~/minimax-images/` directory.

- If user specifies a filename (e.g., `-o cat.png`), it will be saved as `~/minimax-images/cat.png`
- If no filename specified, auto-generates `image_1.png`, `image_2.png`, etc.
- Override with `MINIMAX_OUTPUT_DIR` environment variable
